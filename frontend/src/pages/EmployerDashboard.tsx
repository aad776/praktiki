  ...
  return (
    <div className="min-h-screen bg-slate-50 relative overflow-hidden">
      ...
      <div className="w-full px-2 sm:px-3 lg:px-4 py-3 sm:py-4 relative z-10 space-y-4 flex-grow">
        <header className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 bg-white/80 backdrop-blur-md p-3 sm:p-4 rounded-2xl border border-white/20 shadow-sm">
        ...
        <section className="space-y-4">
          <div className="bg-white/80 backdrop-blur-md p-4 sm:p-5 rounded-2xl border border-white/20 shadow-sm min-h-[400px]">
            ...
            {internships.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-8 text-slate-400">
                ...
                <p className="text-sm">No internships posted yet.</p>
              </div>
            ) : (
              <div className="flex flex-col gap-4">
                {internships.map((job) => (
                  <div key={job.id} className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm hover:shadow-lg transition-all hover:border-blue-200 group">
                    <div className="flex flex-col lg:flex-row gap-4">
                      <div className="flex-1">
                        <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
                          <div className="flex items-center gap-3">
                            <h3 className="font-bold text-xl text-slate-900 leading-tight" title={job.title}>{job.title}</h3>
                            <span className={`px-3 py-1 rounded-lg text-xs font-bold uppercase tracking-wider shrink-0 ${
                          ...
                          <div className="flex items-center gap-4 text-slate-600 text-sm">
                            <div className="flex items-center" title="Duration">
                               <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="mr-1.5 shrink-0"><circle cx="12" cy="12" r="10"></circle><polyline points="12 6 12 12 16 14"></polyline></svg>
                               {job.duration_weeks} wks
                            </div>
                            {job.stipend_amount !== undefined && (
                              <div className="flex items-center font-medium text-slate-800" title="Stipend">
                                ...
                                <span className="text-base">{job.stipend_amount ? job.stipend_amount.toLocaleString() : 'Unpaid'}</span>
                              </div>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="flex items-start text-slate-600 text-sm mb-3">
                          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="mr-2 mt-0.5 shrink-0"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"></path><circle cx="12" cy="10" r="3"></circle></svg>
                          <span className="leading-relaxed text-sm">{job.location}</span>
                        </div>
                      ...
                      <div className="flex lg:flex-col gap-2 justify-center min-w-[160px] border-t lg:border-t-0 lg:border-l border-slate-100 pt-3 lg:pt-0 lg:pl-4">
                        <Link
                          to={`/employer/internship/${job.id}`}
                          className="inline-flex justify-center items-center rounded-xl bg-slate-50 border border-slate-200 px-4 py-2 text-sm font-bold text-slate-700 hover:bg-slate-100 hover:text-slate-900 transition-all w-full"
                        >
                          View Details
                        </Link>
                        <button
                          onClick={() => loadApplications(job.id)}
                          className="inline-flex justify-center items-center rounded-xl bg-blue-50 border border-blue-200 px-4 py-2 text-sm font-bold text-blue-700 hover:bg-blue-100 transition-all w-full"
                        >
                                    ...
                                    <div>
                                      ...
                                      <div className="flex gap-2 mt-3">
                                        {app.student.resume_file_path && (
                                          <a
                                            href={`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/students/resume/download/${app.student.resume_file_path.split('/').pop()}`}
                                            target="_blank"
