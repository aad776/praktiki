...
export function LoginPage() {
  ...
  const handleGoogleSuccess = async (credentialResponse: any) => {
    ...
    try {
      ...
      const data = await api.post<any>("/auth/google", { credential, role });
      
      // Login locally
      const { access_token, role: userRole, is_new_user } = data;
      await login(access_token, userRole);
      
      toast.success("Login successful! 🎉");
      
      // Redirect based on whether it's a new user
      if (is_new_user) {
        navigate(`/${userRole}/setup`);
      } else {
        navigate(`/${userRole}`);
      }
    } catch (err: any) {
