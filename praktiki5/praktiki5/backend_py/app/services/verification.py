...
from app.models import Certificate, TaxonomySkill, Student
from app.schemas import VerificationStatus, EligibilityStatus
...
async def run_verification_workflow(db: AsyncSession, cert: Certificate, student: Student, ai_results: Dict[str, Any]) -> Certificate:
    """
    ...
    cert.identity_verified = verify_identity(extracted_name, student.name, "", student.abc_id, cert.id)
...
async def check_timeline_conflict(db: AsyncSession, student_id: str, start_date: date, end_date: date, exclude_cert_id: str = None) -> bool:
    """
    ...
    if exclude_cert_id:
        query = query.where(Certificate.id != exclude_cert_id)
...
async def map_skills_to_taxonomy(db: AsyncSession, extracted_skills: Dict[str, Any]) -> List[str]:
    """
    Matches extracted skills against the taxonomy_skills table.
    Returns a list of matched taxonomy_codes.
    """
    if not extracted_skills:
        return []
        
    # extracted_skills can be a list or a dict (skill: confidence)
    skill_names = []
    if isinstance(extracted_skills, dict):
        skill_names = list(extracted_skills.keys())
    ...
    # Batch query for all skill names (case-insensitive)
    query = select(TaxonomySkill.taxonomy_code).where(
        or_(*[func.lower(TaxonomySkill.skill_name) == name.lower() for name in skill_names])
    )
    
    result = await db.execute(query)
    mapped_codes = list(result.scalars().all())
    
    if mapped_codes:
        logger.info(f"SKILL_TAXONOMY: Mapped {len(mapped_codes)} taxonomy codes from {len(skill_names)} extracted skills")
