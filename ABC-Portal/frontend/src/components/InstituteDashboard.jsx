...
const InstituteDashboard = () => {
  ...
  const handleBackToMain = () => {
    const mainPortalUrl = import.meta.env.VITE_MAIN_PORTAL_URL || 'http://44.197.97.159';
    window.location.href = `${mainPortalUrl}/institute`;
  };
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-4 sm:p-6 rounded-xl shadow-sm">
        <div className="flex items-center gap-3 w-full sm:w-auto">
          ...
          <button
            ...
            className="text-sm bg-orange-50 text-orange-600 px-3 py-1 rounded-full border border-orange-100 hover:bg-orange-100 transition-colors flex items-center gap-1.5 ml-2"
          >
            Back to Praktiki
          </button>
        </div>
        <div className="flex items-center justify-between sm:justify-end gap-4 w-full sm:w-auto">
          ...
          <div className="relative" ref={notifRef}>
            <button
              ...
              className={`p-2.5 rounded-lg transition-colors relative ${notifOpen ? 'bg-orange-100 text-orange-700' : 'text-gray-500 hover:bg-gray-100'}`}
            >
              <Bell size={24} />
              {unreadCount > 0 && (
