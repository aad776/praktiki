...
class TPExtractor:
    ...
    def _extract_person_fallback(self, text):
        ...
        name_patterns = [
            r'(?:certify that|presented to|awarded to|is given to|this is to certify that|recognizes)\s*\n*\s*([A-Z][a-z\s]{2,}(?:\s+[A-Z][a-z\s]{2,})+)',
            r'([A-Z][a-z\s]{2,}(?:\s+[A-Z][a-z\s]{2,})+)\s*\n*\s*(?:has successfully|has completed|completed)',
        ]
    ...
    def _extract_org(self, doc):
        ...
        if orgs:
            return {'value': orgs[0].strip(), 'conf': 0.75}
    ...
    def _extract_org_fallback(self, text):
        ...
        org_patterns = [
            r'(?:Organization|Company|Institution|For|At)[:\s,]+([A-Z][a-zA-Z&\s\(\)\-\.]{3,})',
            r'([A-Z][a-zA-Z&\s\-]{3,})\s+(?:certifies that|presents this|certify that|confers)',
            r'(?:by|from|at|For)\s+([A-Z][a-zA-Z&\s\(\)\-]{3,}(?:Inc\.|Ltd\.|LLC|Corp\.|School|Academy|Foundation|NGO|Trust|University))'
        ]
        
        common_words = ["We", "The", "This", "To", "For", "By", "At", "From", "He", "She", "It", "They", "During", "Certificate"]
        ...
        for line in lines[:6]:
            ...
            if any(kw in clean_line.upper() for kw in ["SCHOOL", "TRUST", "INSTITUTE", "UNIVERSITY", "LIMITED", "LTD", "INC", "FOUNDATION", "CORPORATION", "ACADEMY"]):
                if len(clean_line) > 3:
        ...
        if lines and len(lines[0]) > 3 and len(lines[0]) < 60:
    ...
    def _extract_title(self, text):
        ...
        m = re.search(r'internship\s+(?:in|as|position of|with us as a)\s*\n*\s*([A-Za-z\-\s]{4,50})', text, re.I)
        if m:
            title = m.group(1).split('\n')[0].strip()
            return {'value': title, 'conf': 0.8}
            
        m = re.search(r'([A-Z][a-z\s\-]+(?:Intern|Trainee|Assistant|Developer|Designer|Analyst|Program|Writer))', text)
        if m:
            title = m.group(1).split('\n')[0].strip()
            return {'value': title, 'conf': 0.7}
